...
def crop_candidate(frame, foreground, bbox, refine=False, candidate_mask=None):
    ...
    if candidate_mask is not None:
        # Avoid admitting another component simply because it is inside the bbox.
        mask = candidate_mask[y1-y:y2-y, x1-x:x2-x].copy()
        if mask.shape != crop.shape[:2]:
            raise ValueError("Candidate mask and bounding box disagree")
